...
To include clojure-control,add:
   		 ...
   		 [control "0.2.3"]  ;clojure 1.2
		 [control "o.2.4"]  ;clojure 1.3

But the lein plugin is recommended.
...
and run `lein deps` to resolve it. Or you can install it by lein:

		       lein plugin install control 0.2.3 [or 0.2.4 for clojure 1.3]    
...
Get the current date from the two machines listed in the 'mycluster' config with a single command,edit `control.clj` generated by `lein control init`:
...
Run with
		...
		lein control run mycluster date
